...
**Note**: Streaming responses have been fully implemented across all LLM providers. All providers now use true streaming with SSE/NDJSON parsing.

## Implementation Tasks

### Phase 1: PromptViewModel Streaming Integration ✅

- [x] **Task 1.1**: Update `generate(with:)` to use streaming
...
- [x] **Task 2.1**: Modify request building to enable streaming
  - Add `"stream": true` to request JSON body
  - Use `URLSession.bytes(for:)` instead of `URLSession.data(for:)`

- [x] **Task 2.2**: Implement SSE parsing for OpenAI format
  - Parse `data: {...}` lines
...
- [x] **Task 3.1**: Modify request building to enable streaming
  - Add `"stream": true` to request JSON body
  - Use `URLSession.bytes(for:)` instead of `URLSession.data(for:)`

- [x] **Task 3.2**: Implement SSE parsing for Anthropic format
...
- [x] **Task 7.1**: Manual testing with each provider
