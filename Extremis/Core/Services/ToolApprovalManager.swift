...
final class ToolApprovalManager {
    ...
    /// All decisions made this application session (for audit log)
    ...
    func requestApproval(
        ...
        if pendingRequests.isEmpty {
            recordDecisions(sessionApprovedDecisions)
        ...
        let (userDecisions, allowAllOnce) = await waitForUserDecisions(pendingRequests)

        // Combine session and user decisions
        let allDecisions = sessionApprovedDecisions + userDecisions.values
        recordDecisions(Array(userDecisions.values))
    ...
    private func waitForUserDecisions(
        ...
        return await withTaskCancellationHandler {
            await withCheckedContinuation { continuation in
                ...
                delegate.showApprovalUI(for: requests) { uiResult in
                    Task {
                        let canResume = await tracker.tryResume()
                        if canResume {
                            timeoutTask.cancel()
            ...
            // If the task is cancelled, dismiss the UI
            Task { @MainActor in
                self.uiDelegate?.dismissApprovalUI()
            }
        }
    }
    ...
    /// Record decisions for audit log
    func recordDecisions(_ decisions: [ApprovalDecision]) {
        sessionDecisions.append(contentsOf: decisions)

        // Log for debugging
        for decision in decisions {
            let emoji = decision.action == .approved || decision.action == .sessionApproved ? "✅" : "❌"
            print("\(emoji) \(decision.action.rawValue): \(decision.toolName) (\(decision.connectorId))")
        }
    }

    /// Clear decision log (e.g., on app restart or session clear)
    func clearDecisionLog() {
        sessionDecisions.removeAll()
        print("📋 Cleared approval decision log")
    }
